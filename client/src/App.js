...
function App() {
  ...
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false); // Stare pentru meniul de mobil
  ...
  return (
    <BrowserRouter>
      ...
      {/* OVERLAY-UL PENTRU MENIU MOBIL (Se face negru in spate) */}
      <div 
        className={`drawer-overlay ${isMobileMenuOpen ? 'open' : ''}`} 
        onClick={closeMenu}
      ></div>

      {/* MENIUL LATERAL PENTRU MOBIL */}
      ...
      {/* BARA DE NAVIGARE PRINCIPALA (NAVBAR) */}
      <nav style={{ 
        padding: '10px 30px',
        ...
        {/* MENIUL DESKTOP (Se ascunde pe mobil) */}
        <div className="desktop-only" style={{ flex: 2, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '20px' }}>
          <Link to="/marketplace" className="nav-link" style={{ fontSize: '14px', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none' }}>Marketplace</Link>
          <Link to="/leaderboard" className="nav-link nav-link-top" style={{ fontSize: '14px', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none' }}>Top</Link>

          {user && (
            <>
              <Link to="/upload" className="nav-link" style={{ fontSize: '14px', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none' }}>Upload</Link>
              <Link to="/sessions" className="nav-link" style={{ fontSize: '14px', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none' }}>Meditații</Link>
              <Link to="/profile" className="nav-link" style={{ fontSize: '14px', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none' }}>Profil</Link>
              <Link to="/learn" className="nav-link" style={{ fontSize: '14px', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none', color: '#d2b4de', fontWeight: 'bold' }}>AI Tutor</Link>
              {user.role === 'admin' && <Link to="/admin" className="nav-link" style={{ fontSize: '14px', color: '#ff4757', fontWeight: 'bold', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none' }}>Admin</Link>}
            </>
          )}
        </div>
        ...
        <div className="desktop-only" style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '15px' }}>
          {user ? (
            ...
            <>
              <Link to="/login" className="nav-link" style={{ fontSize: '14px', textShadow: isScrolled ? '0px 2px 4px rgba(0,0,0,0.5)' : 'none' }}>Login</Link>
              <Link to="/register" style={{ background: 'linear-gradient(135deg, #27ae60, #2ecc71)', color: 'white', padding: '6px 16px', borderRadius: '8px', fontSize: '14px', textDecoration: 'none', fontWeight: 'bold', boxShadow: '0 4px 10px rgba(39, 174, 96, 0.3)' }}>Register</Link>
            </>
          )}
        </div>
